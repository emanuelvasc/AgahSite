...
import { PageHeader, Button, SearchInput, StatusBadge, Modal, Input, Card, Avatar } from '../../components/ui';
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Button icon={Plus} onClick={() => setShowModal(true)}>Novo Fornecedor</Button>}
      />

      <div className="flex items-center gap-3 mb-6">
        <SearchInput value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar fornecedor..." className="max-w-sm" />
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        {filtered.map((supplier, i) => (
          <motion.div key={supplier.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
            className="glass rounded-2xl p-6 card-hover cursor-pointer" onClick={() => setSelected(supplier)}>
            <div className="flex items-start gap-4 mb-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="font-semibold text-white font-display">{supplier.name}</h3>
                  <StatusBadge status={supplier.status} />
                </div>
                <div className="text-xs text-orange-400 mt-0.5">{supplier.category}</div>
              </div>
            </div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Novo Fornecedor">
        <div className="space-y-4">
          ...
          <Input label="Categoria" placeholder="Tecidos, Aviamentos..." required />
