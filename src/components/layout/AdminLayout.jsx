...
import {
  LayoutDashboard, Users, Package, Boxes, ShoppingBag, ClipboardList,
  Factory, Truck, BarChart3, MessageSquare, Settings, LogOut,
      ...
      >
        ...
        {!collapsed && (
          <div className="px-4 pt-3 pb-1">
            <span className="text-[10px] font-semibold text-orange-400/70 uppercase tracking-widest">Painel Admin</span>
          </div>
        )}
        ...
        <nav className="flex-1 overflow-y-auto py-2 space-y-0.5 px-2">
          {navItems.map(({ to, label, icon: Icon, end }) => (
            ...
            >
              {({ isActive }) => (
                <>
                  <Icon size={18} className={isActive ? 'text-orange-400 flex-shrink-0' : 'flex-shrink-0'} />
                  <AnimatePresence>
                    {!collapsed && (
                      <motion.span
                        initial={{ opacity: 0 }}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-16 flex items-center justify-between px-6 border-b border-white/6 flex-shrink-0" style={{ background: '#0f0f1a' }}>
          <div className="text-sm text-slate-500">
            <span className="text-orange-400">AGAH</span> <span className="text-slate-600">/</span> Admin
          </div>
